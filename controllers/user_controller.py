...
def index(id: int):
    try:
      # FETCH USER
        ...
        flash('Error viewing user. Please try again later', 'danger')
...
def create_report(user_id: int):
    try:
        ...
        flash('Card reports created successfully', 'success')
        return(redirect(url_for('.index', id=user_id)))

    except:
        flash('Error creating card reports. Please try again later', 'danger')
        return(redirect(url_for('.index', id=user_id)))
